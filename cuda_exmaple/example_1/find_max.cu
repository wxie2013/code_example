...
int main(void)
{
    ...
    cudaOccupancyMaxPotentialBlockSize( &minGridSize, &blockSize, find_max, 0, Nx+Ny);
    ...
    int maxActiveBlocks;
    cudaOccupancyMaxActiveBlocksPerMultiprocessor( &maxActiveBlocks, find_max, blockSize, 0);
    ...
    tot = 0;
	find_max<<<gridSize_3D, blockSize_3D>>>(Nx, Ny, d_tot, d_x, d_y);
