                    ...
                    for(std::vector<pat::GenericParticle>::const_iterator tk_it=input_tracks.begin();
                        ...
                        */
						for(int d=0; d < DInfo.size; d++){
							...
							/*//since we only have 3 tracks for Lc
							...
							*/
/*//since we dp not have resonance partocles.
                        ...
                        //if(dropUnusedTracks_ && listOfRelativeDCand1.size() == 0 && listOfRelativeDCand2.size() == 0 && listOfRelativeDCand3.size() == 0 && listOfRelativeDCand4.size() == 0 && listOfRelativeDCand5.size() == 0 && listOfRelativeDResCand1.size() == 0 && listOfRelativeDResCand2.size() == 0 && listOfRelativeDResCand3.size() == 0 && listOfRelativeDResCand4.size() == 0) continue;//drop unused tracks old justification, which still have resonance particles
						...
						//TrackInfo.fpbarrelhit    [TrackInfo.size] = tk_it->track()->hitPattern().hasValidHitInFirstPixelBarrel();//old
						//TrackInfo.fpendcaphit    [TrackInfo.size] = tk_it->track()->hitPattern().hasValidHitInFirstPixelEndcap();//old
						TrackInfo.fpbarrelhit    [TrackInfo.size] = tk_it->track()->hitPattern().hasValidHitInPixelLayer(PixelSubdetector::PixelBarrel,1);//new
						TrackInfo.fpendcaphit    [TrackInfo.size] = tk_it->track()->hitPattern().hasValidHitInPixelLayer(PixelSubdetector::PixelEndcap,1);//new
                        ...
                        /*// since PbPb we do not use dedx, I delect this variable.
						if(readDedx_) {
...
//cout<<"genTrack"<<genTrackPtr [TrackInfo.size]<<endl;
						...
						*/
/*//since we do not have resonance particle, we remove it
